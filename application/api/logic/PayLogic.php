...
class PayLogic
{
    ...
    function __construct($openid="",$out_trade_no="",$total_fee=0,$body="下单消费",$notify_url="") {
        ...
        $this->openid = $openid;
        $this->mch_id = '1523799431'; //1523799431
        $this->key = 'IRiQS5AzukkVWGrtPWnKg8JyzmnC58pA';//api 密钥
        $this->out_trade_no = $out_trade_no;
        $this->total_fee = $total_fee; //支付金额
        $this->body = $body;
...
//         	    var_dump($this->tui_url);
    ...
    protected function unifiedorder() {
        ...
        if(!isset($this->out_trade_no)) {
            return returnBad("缺少统一支付接口必填参数out_trade_no！");
        }else if(!isset($this->body)){
            return returnBad("缺少统一支付接口必填参数body！");
        }else if(!isset($this->total_fee)) {
            return returnBad("缺少统一支付接口必填参数total_fee！");
        }
