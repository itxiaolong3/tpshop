...
//        	}
	...
	*/
     ...
     * @function musicUp
     */
    public function musicUp()
    {
        // 上传音频框中的描述表单名称，
        $pictitle = I('musictext');
        $dir = I('dir');
        $title = htmlspecialchars($pictitle , ENT_QUOTES);
        $path = htmlspecialchars($dir, ENT_QUOTES);
        // 获取表单上传文件
...
//        $result = $this->validate(
//            ['file' => $file],
//            ['file'=>'audio|fileSize:40000000|fileExt:jpg,jpeg,gif,png,mp3'],
//            ['file.audio' => '上传文件必须为图片','file.fileSize' => '上传文件过大','file.fileExt'=>'上传文件后缀名必须为jpg,jpeg,gif,png,mp3']
//        );
        //var_dump($file);die();
        $upload_max_filesize = @ini_get('file_uploads') ? ini_get('upload_max_filesize') :'unknown';
        if (true !== $result || !$file) {
            $state = "ERROR 音频过大, 最大不能超过: $upload_max_filesize";
        } else {
            $return = $editor->saveUploadImage($file, $this->savePath);
            $state = $return['state'];
            $return_data['url'] = $return['url'];
