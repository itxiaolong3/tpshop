...
class User
{
    ...
    function updateUserLevelByW($user_id)
    {
        ...
        if($level<7){ //6
            $total_amount = Db::name('order')->master()->where(['user_id' => $user_id, 'pay_status' => 1, 'order_status' => ['NOTIN', [3, 5]]])->sum('order_amount'); //order_status 3：已取消 5 已作废
            // 客户没添加用户等级，上报没有累计消费的bug
            $level_info = Db::name('user_level')->field(['amount,level_id,discount'])->select();
            if($level_info[0]['amount']>$total_amount){
                $update['level'] = $level_info[0]['level_id'];
                $update['discount'] = $level_info[0]['discount'] / 100;
            }elseif($level_info[0]['amount']<=$total_amount &&$total_amount<$level_info[1]['amount']){
                $update['level'] = $level_info[1]['level_id'];
                $update['discount'] = $level_info[1]['discount'] / 100;
            }elseif($level_info[1]['amount']<=$total_amount &&$total_amount<$level_info[2]['amount']){
                $update['level'] = $level_info[2]['level_id'];
                $update['discount'] = $level_info[2]['discount'] / 100;
            }elseif($level_info[2]['amount']<=$total_amount &&$total_amount<$level_info[3]['amount']){
                $update['level'] = $level_info[3]['level_id'];
                $update['discount'] = $level_info[3]['discount'] / 100;
            }elseif($level_info[3]['amount']<=$total_amount &&$total_amount<$level_info[4]['amount']){
                $update['level'] = $level_info[4]['level_id'];
                $update['discount'] = $level_info[4]['discount'] / 100;
            }elseif($level_info[4]['amount']<=$total_amount &&$total_amount<$level_info[5]['amount']){
                $update['level'] = $level_info[5]['level_id'];
                $update['discount'] = $level_info[5]['discount'] / 100;
            }elseif($level_info[5]['amount']<=$total_amount &&$total_amount<$level_info[6]['amount']){
                $update['level'] = $level_info[6]['level_id'];
                $update['discount'] = $level_info[6]['discount'] / 100;
            }
