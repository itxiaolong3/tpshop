...
class CartLogic extends Model
{
    ...
    public function setSpecGoodsPriceById($item_id)
    {
        if ($item_id > 0) {
            ...
            $this->specGoodsPrice = $specGoodsPriceModel::get($item_id, '', 10); //规格商品价格表
        }else{
...
//            if($this->goods['virtual_indate'] < time()){  //商品截至时间小于当前时间
...
//            'virtual_indate'=>$this->goods['virtual_indate'],
        ...
        $buyGoods['cut_fee'] = $cart->getCutFeeAttr(0, $buyGoods);//商品总额优惠
        $buyGoods['goods_fee'] = $cart->getGoodsFeeAttr(0, $buyGoods);//商品运费
        $buyGoods['total_fee'] = $cart->getTotalFeeAttr(0, $buyGoods);//总共运费
        return $buyGoods;
    }
...
//        }
        ...
        if($this->specGoodsPrice){
            ...
            $prom_type = $this->goods['prom_type']; //0默认1抢购2团购3优惠促销4预售5虚拟(5其实没用)6拼团7搭配购
        }
        switch($prom_type) {
            case 1:
    ...
    private function addNormalCart()
    {
        if (empty($this->specGoodsPrice)) {
            ...
            $store_count = $this->specGoodsPrice['store_count']; //存储数量 总数量
        }
        // 查询购物车是否已经存在这商品
       ...
       $goods_info= M('cart')->where('goods_id',$this->goods['goods_id'])->where('item_id',$this->specGoodsPrice['item_id'])->find();
       if(!empty($goods_info)){
         $result=  M('cart')->where('goods_id',$this->goods['goods_id'])->where('item_id',$this->specGoodsPrice['item_id'])->setInc('goods_num',$this->goodsBuyNum);
           if ($result === false) {
...
//        halt(($promGoodsLogic->checkActivityIsEnd() || !$promGoodsLogic->checkActivityIsAble()));
    ...
    public function getCartList($selected = 0)
    {
        ...
        $cartList = $cart->with('goods')->where($cartWhere)->select();  // 获取购物车商品 与goods表关联 查询cart表数据
    ...
    public function getCartListBycartId1($cart_ids = 0)
    {
        $cart = new Cart();
        // 如果用户已经登录则按照用户id查询
        if ($this->user_id) {
        ...
        $cartWhere['id'] = ['in', $cart_ids];
        $cartWhere['combination_group_id'] = 0;
      //  M('cart')->alias('c')->join('goods g','c.goods_id =g.goods_id','right')->field(['user_id,goods_id,goods_name,market_price'])
        $cartList = $cart->with('goods')->where($cartWhere)->select();  // 获取购物车商品
    ...
    public function changeNum($cart_id, $goods_num)
    {
        ...
        $store_count = db('spec_goods_price')->where(['item_id'=>$cart['id'],'goods_id'=>$cart['goods_id']])->value('store_count');
        if(!$store_count){
        ...
        if($goods_num<0){
            if($cart['goods_num']<abs($goods_num)){
                return ['status' => 0, 'msg' => "删减的数量超出购物车数量" ];
            }
        }
        ...
        if ($cart['prom_type'] == 0) {
          ...
          //  echo $goods_num;die;
            if($goods_num<0){
              ...
              $res=  M('cart')->where(['id'=>$cart_id])->setdec('goods_num',$goods_num); //减
              if($res){
                  $ress['status']=1;
                  $ress['msg']='扣减成功！';
                  return $ress;
                ...
                $res=  M('cart')->where(['id'=>$cart_id])->setinc('goods_num',$goods_num); //减
                if($res){
                    $ress['status']=1;
                    $ress['msg']='扣减成功！';
                    return $ress;
...
//            $carts = $Cart->where(['combination_group_id' => $cart['combination_group_id'], 'id' => ['neq', $cart['id']]])->select();
        ...
        $cart->save();
        return ['status' => 1, 'msg' => '修改商品数量成功', 'result' => '修改成功'];
    }
