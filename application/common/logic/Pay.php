...
class Pay
{
    ...
    public function useUserMoney($user_money)
    {
        ...
        if($user_money > 0){
            ...
            if($this->orderAmount > 0){
                if($user_money > $this->orderAmount){ //使用余额  账单金额
                    $this->userMoney = $this->orderAmount;
                    $this->orderAmount = 0;
                }else{
                    $this->userMoney = $user_money; //使用金额
                    $this->orderAmount = $this->orderAmount - $this->userMoney;//余额
     ...
     * @return $this
     */
    public function useUserMoneykou($user_money)
    {
        if($user_money > 0){
            if($user_money > $this->user['user_money']){
                throw new TpshopException("计算订单价格",0,['status' => -6, 'msg' =>  "你的账户可用余额为:" . $this->user['user_money'], 'result' => '']);
    ...
    public function useCouponById($coupon_id){
        if($coupon_id > 0){
            ...
            $userCoupon = $couponList->where(['uid'=>$this->user['user_id'],'id'=>$coupon_id])->select(false);
            var_dump($userCoupon);die;
            if($userCoupon){
                ...
                if($coupon){
                    ...
                    if ($this->orderAmount > 0) {
                        if ($coupon['money'] > $this->orderAmount) {
                            $this->couponPrice = $this->orderAmount; //优惠券抵消金额
                            $this->orderAmount = 0; //应付金额
                        } else {
     ...
     * @return $this
     */
    public function useCouponlistById($coupon_id){
        if($coupon_id > 0){
//            $couponList = new CouponList();
//            $userCoupon = $couponList->where(['uid'=>$this->user['user_id'],'id'=>$coupon_id])->select(false);
            $userCoupon= M('prom_coupon')->where(['uid'=>$this->user['user_id'],'id'=>$coupon_id])->find();
            if($userCoupon){
                $coupon = Db::name('prom_order')->where(['id'=>$userCoupon['poid'],'status'=>1])->find(); // 获取有效优惠券类型表
                if($coupon){
                    $this->couponId = $coupon_id; //优惠券id

                    if ($this->orderAmount > 0) {
                        if ($coupon['condition_money'] > $this->orderAmount) { //条件价》订单加
                            $this->couponPrice = 0; ////优惠券抵消金额
                            $this->orderAmount = $this->orderAmount;//应付金额
                            $this->orderPromId= $coupon['id'];
                        } else {
                            $this->couponPrice = $coupon['expression'];
                            $this->orderAmount = $this->orderAmount - $this->couponPrice;
                            $this->orderPromId= $coupon['id'];
                        }
                    }
