...
class PlaceOrder
{
    ...
    public function addNormalOrder($pwd) //生成订单
    {
        $this->check();
        $this->queueInc(); //队列 排队一百人
        $this->addOrder(); //增加订单
        $this->addOrderGoods();
        $this->addShopOrder();
        ...
        $this->deductionCouponlist();//扣除优惠券
        $this->changUserPointMoney($this->order,$pwd);//扣除用户积分余额
        $this->queueDec(); //结算订单排队
    }
...
//            }
           // echo encrypt($this->payPsw);die;
             // var_dump($this->pay->getUser()['paypwd']);die;
//              $user_pwd=$this->pay->getUser()['paypwd'];
//            if (encrypt($pwd) !== $user_pwd) {
//                echo 111;die;
...
//        }
     ...
     * 扣除优惠券
     */
    public function deductionCouponlist()
    {
        $couponId = $this->pay->getCouponId();
        if($couponId > 0){
            $user = $this->pay->getUser();
//            $couponList = new CouponList();
//            $userCoupon = $couponList->where(['status'=>0,'id'=>$couponId])->find();
            $userCoupon= M('prom_coupon')->where(['use_status'=>0,'id'=>$couponId])->find();
            if($userCoupon){
                $data['uid'] = $user['user_id'];
                $data['order_id'] = $this->order['order_id'];
                $data['use_time'] = time();
                $data['use_status'] =  1;
               M("prom_coupon")->where(['id'=>$userCoupon['id']])->save($data);
                Db::name('prom_coupon')->where('id', $userCoupon['id'])->setInc('use_num');// 优惠券的使用数量加一
            }
        }
    }
     ...
     * @param Order $order
     */
    public function changUserPointMoney(Order $order,$pwd)
    {

           if(empty($pwd)){
               return returnBad("缺少支付密码，无法进行积分或元宝操作");
           }
            $user=$this->pay->getUser();
           //echo $user['paypwd'].'+++'.encrypt($pwd);die;
            if(empty($user['paypwd']) || $user['paypwd']==null){
                return returnBad('请去设置支付密码');

            }
            if (encrypt($pwd) != $user['paypwd']) {
                throw new TpshopException('提交订单', 0, ['status' => -8, 'msg' => '支付密码错误', 'result' => '']);
            }
