...
class UsersLogic extends Model
{
    ...
    public function getuserinfo($user_id){
        ...
        $user = M('users')->where('user_id', $user_id)->field('mobile')->find();
        if (!$user) {
            return false;
        }
        return $user;
    ...
    public function get_info($user_id)
    {
        ...
        $user = M('users')->where('user_id', $user_id)->field(['user_no,user_money,pay_points,head_pic,nickname,level'])->find();

        if (empty($user)) {
            return false;
        }else{
            $user['head_pic']=url_add_domain($user['head_pic']);
    ...
    public function get_comment($user_id,$status=2){
        if($status == 1){
            ...
            if($status == 0){
                $comment_where['og.is_comment'] = 0;//没有发货
                $comment_where['o.order_status'] = 2; //订单已发货
            }
...
//            return array('status'=>-1,'msg'=>'原密码验证失败','result'=>'');
     ...
     * @return array
     */
    public function payPwd($user_id,$pwd,$new_password,$confirm_password){
        $data=[
            'password' => $pwd,
            'paypwd' =>$new_password,
            'paypwd2' =>$confirm_password,
        ];
        $UserRegvalidate = Loader::validate('UserPay');
        if(!$UserRegvalidate->scene('set_paypwd')->check($data)){
            return array('status'=>-1,'msg'=>$UserRegvalidate->getError(),'result'=>'');
        }
        $password = M('users')->where("user_id",$user_id)->field('password')->find();

        if($password['password'] !=encrypt($pwd)){
            return array('status'=>-1,'msg'=>'登录密码不正确',);
        }
        if($data['paypwd']!=$data['paypwd2']){
            return array('status'=>-1,'msg'=>'两次输入的密码不一致',);
        }
        $row = M('users')->where("user_id",$user_id)->update(array('paypwd'=>encrypt($new_password)));//增加密码
        if(!$row){
    ...
    */
    ...
    public function addUserSign($user_id, $date)
    {
        ...
        $data['sign_total'] = 1; //次数
        $data['sign_last'] = $date;//最后签到时间
        $data['cumtrapz'] = $config['sign_integral'];//签到积分
        $data['sign_time'] = "$date";
        $data['sign_count'] = 1;
