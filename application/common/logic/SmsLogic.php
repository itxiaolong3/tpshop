...
class SmsLogic 
{
    ...
    public function sendSms($scene, $sender, $params, $unique_id=0)
    {
        ...
        if ($sender != '' && check_mobile($sender)) {//如果是正常的手机号码才发送
            ...
            if ($resp['status'] == 1) {
                M('sms_log')->where(array('id' => $log_id))->save(array('status' => 1)); //修改发送状态为成功 save更新 add是添加
            }else{
                M('sms_log')->where(array('id' => $log_id))->update(array('error_msg'=>$resp['msg'])); //发送失败, 将发送失败信息保存数据库 update 更改
            }
            return $resp;
        } else {
    ...
    public function sendMsg($params, $unique_id=0)
    {
        ...
        $sender = $params['sender'];//发送者手机号
        $msg = $params['msg']; //发送内容
        $scene = $params['mmt_code']; //发送场景
        if(empty($unique_id)){
            $session_id = session_id();
        }else{
            $session_id = $unique_id;
