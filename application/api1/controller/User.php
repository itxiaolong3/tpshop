...
//            return returnBad('您还不是分销商',308);
        $level = I('post.level',1);//等级 这个等级是根据用户消费积分来的 默认为下级
        $q = I('post.q','','trim');
    ...
    public function order_list(){
        $this->user_id=207;
        $p =  $type = I('type','1');//订单商品？？？
        $user = Db::name('users')->where(['user_id'=>$this->user_id])->find();
//        if($user['is_distribut'] != 1)
//            return returnBad('您还不是分销商');
        $status = I('post.status','all');//订单状态
        if($status=='all'){
            $status = '0,1,2,3';
        }
        ...
        if($p==1){
            ...
            $data['distribut_money'] = $user['distribut_money']; //分佣总金额 元宝总数
            $data['total_order_money'] = $subtotal;
        }
        foreach ($list as $k => $v) {
            $list[$k]['head_pic'] = $userList[$v['buy_user_id']]['head_pic'];
            $list[$k]['status'] = C('REBATE_STATUS')[$v['status']];
            $list[$k]['level'] = C('LEVEL_STATUS')[$v['level']];
    ...
    public function address_list()
    {
        ...
        $address_lists =  db('user_address')->where('user_id', $user_id)->select();
    ...
    */
    public function receiving_address(){
        ...
        $arr=[];
        if(!empty($user_id)){
         $result= M("users")->where(array('user_id'=>$user_id))->find();
         if(!empty($result)){
           $arr=array(
               "user_id"=>$user_id,
               "consignee"=>$name,
           ...
           if($is_default==1){
               $is_default=0;
               M('user_address')->where(array('user_id'=>$user_id))->save(array('is_default'=>$is_default));
           }
           M('user_address')->where(array('user_id'=>$user_id))->add($arr);
     ...
     public function add_collect(){
         ...
         if(!empty($uid)){
             if(empty($goods_id)){
                 ...
                 if(!empty($result)){
                     if(M('goods_collect')->where(array('user_id'=>$uid,'goods_id'=>$goods_id))->find()){
                         return  returnOk(['status' => 1, 'msg' => '商品已收藏过']);
                     }else{
                         $result= M('goods_collect')->add(array('user_id'=>$uid,'goods_id'=>$goods_id,'add_time'=>time()));
                         if($result){
    ...
    public function get_coupon(){
       ...
       // $user_id = $this->user_id; 在公众号中获取的用户user_id
        ...
        $coupon_list = $activityLogic->get_coupon($id, $user_id);
        return returnOk($coupon_list);
    }
    /**
     * 用户领取优惠券列表
    ...
    public function coupon_list(){
        ...
        if(!empty($user_id)){
            $activityLogic = new \app\common\logic\ActivityLogic;
            $coupon_list = $activityLogic->getCouponListInfo($user_id);
            return returnOk($coupon_list);
        }else{
    ...
    public function user_sign()
    {
        ...
        //是否正确请求
        if((date("Y-n-j", time()) != $date)) return returnBad("签到失败",306);
        //签到开关
